...
(deftemplate ComplexQueryInput
   ...
   (slot year) ; Assuming 'none' is a special value for the year
)
...
(defrule get-user-input
   ...
   (printout t "8. Recommend based on multiple criteria" crlf)
   (printout t "9. Exit program" crlf)
   (printout t "======================================================" crlf)
...
; Cleanup rule to retract GenreFound and GenreSearchStarted facts
...
; Cleanup rule to retract ArtistFound and ArtistSearchStarted facts
...
; Case 5, recommend song by user input popularity
(defrule recommend-by-popularity
   ?ui <- (UserInput (value 5))
   =>
   (printout t "How popular of a song would you like to discover? Enter a number between 50 (less popular) - 100 (more popular): ")
   (bind ?popularity (read))
   (printout t crlf crlf)
   (printout t "Recommended songs based on user input popularity:" crlf)
   (do-for-all-facts ((?m Music)) 
      (and (>= ?m:popularity ?popularity) (<= ?m:popularity ?popularity))
      (printout t "Title: " ?m:title ", Artist: " ?m:artist crlf))
)

; Recommend song by predefined popularity range
(defrule recommend-by-predefined-popularity-range
   ?pr <- (PopularityRange (lower ?lower) (upper ?upper))
   =>
   (printout t "Recommended songs based on predefined popularity range:" crlf)
   (do-for-all-facts ((?m Music)) 
...
; case 8, recommend based on multiple criteria 
...
(defrule recommend-based-on-complex-query
   ...
   (if (> (length$ ?songs) 0)
      ...
      (printout t "Recommended song: " (fact-slot-value ?selectedSong title) ", Artist: " (fact-slot-value ?selectedSong artist) ", Year: " (fact-slot-value ?selectedSong year) crlf)
      else
      (assert (PopularityRange (lower 50) (upper 100)))
   )
   (retract ?cq)
)


; Fallback rule to recommend song by popularity if no match is found
(defrule recommend-by-popularity-if-no-match
   (PopularityRange (lower ?lower) (upper ?upper))
   =>
   (bind ?songs (find-all-facts ((?m Music))
                (and (>= ?m:popularity ?lower) (<= ?m:popularity ?upper))))
   (if (> (length$ ?songs) 0)
      ...
      (bind ?randomIndex (random 1 (length$ ?songs)))
      (bind ?selectedSong (nth$ ?randomIndex ?songs))
      (printout t "Recommended song based on popularity: " (fact-slot-value ?selectedSong title) ", Artist: " (fact-slot-value ?selectedSong artist) crlf)
   )
)
...
(defrule invalide-choice
   (UserInput (value ?UserInput&:(or (< ?UserInput 1) (> ?UserInput 9))))
   =>
   (printout t "Invalid Choice! Please restart and enter a number 1-9." crlf)
   (halt)
)